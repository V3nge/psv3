...
async function loadAllGames() {
    ...
    if (ToSearch == null) {
        var built = listing
            .map(
                (game) =>
                `<div class="carousel-element centered"><a href='javascript:loadGame("/games/${game.slug}")'><img src="${game.thumbnail}" class="thumbnail" loading="lazy"></img><text class ="centerthing">${game.name}</text></a></div>`
            )
            .join("");
